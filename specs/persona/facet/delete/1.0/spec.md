...
issuedAtRequirement:
  ...
  rationale: >-
    ...
    against a facetId the holder has since reused would remove an arrangement
    they had just rebuilt, and §7.2 item 11 can only absorb the duplicate inside
    a bounded window.
sideEffects:
  level: destructive
  rationale: >-
    Removes one facet permanently. Declared destructive because the record cannot
...
exposure:
  ...
  rationale: >-
    ...
    count of the profiles that no longer belong to any facet.
errorCodes:
  - code: persona/facet/delete:versionConflict
    meaning: The `expectedVersion` precondition failed — the facet changed after the caller read it. Details carry the maintainer's current version. Nothing is deleted.
    retryable: false
related:
  - persona/facet/put
  - persona/facet/list
...
The whole of its design is one sentence: **a facet is an arrangement, not a
...
untouched, and a holder who deletes a facet by mistake has lost a few minutes of
...
The authority is **ownership of the facet**, exactly as for `persona/facet/put`:
...
**`facetId`** — the facet to remove.

**`expectedVersion`** — OPTIONAL precondition. Supply the version a prior read
returned to refuse a delete against a facet that has changed since it was looked
...
**`releasedFaces`** — how many profiles now belong to no facet. Nothing was
...
attribute named by the facet's membership. Deleting a facet is not a cascade and
...
### Deleting a facet the caller has just read
...
a boolean and a count. Neither carries a facet name, a value, or anything about
...
own store. The facet whose deletion this records was itself agent-scoped and
never crossed into any trust context, so its removal is invisible everywhere a
verifier can see.

### Retention

The facet is removed. A maintainer **SHOULD NOT** retain a deleted facet — it
has no evidentiary value, since nothing about a facet is ever presented to a
