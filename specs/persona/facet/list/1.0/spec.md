...
exposure:
  ...
  rationale: >-
    ...
    is why a facet never leaves the agent scope.
errorCodes:
...
It is the read half of `persona/facet/put`, and it exists as its own task rather
than as a member on `persona/profile/list` for the reason the two are separate
records: a facet groups attributes as well as profiles, and a listing that
...
`persona/facet/put`: the issuer must be the holder whose facets these are, as
the maintainer identifies them. An administrator scoped to a single trust
context is **not** authorized — a facet spans every context, so an authority
...
cap on the result.** A producer wanting every facet follows `nextCursor` to the
...
**`nextCursor`** — present when more facets remain. **Its absence is the only
...
**`Facet`** — one named part of the holder's life. See
[`persona/facet/put`](../../put/1.0/spec.md) for the meaning of each member.
...
### Two facets, and no more to come
...
The response carries every facet `name` the holder has chosen and the membership
...
membership, because a facet is small and a second shape would be a second thing
...
**agent-scoped and stays there**: a facet **MUST NOT** appear in a materialised
...
record its own policy keeps, and such a record **MUST NOT** include facet names.
...
authority — is outside it. A facet is not a permission.
